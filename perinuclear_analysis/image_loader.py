...
from .utils import validate_file_path, get_pixel_size_from_metadata
...
class ImageLoader:
    ...
    def load_image(self, filepath: Union[str, Path], force_convert: bool = None) -> np.ndarray:
        ...
        if suffix == '.nd2' and should_convert:
            # Convert ND2 to TIFF
            logger.info(f"Converting {filepath.name} to TIFF format for pipeline compatibility")
            
            # Check if already converted
            tiff_path = filepath.with_suffix('.tiff')
            if tiff_path.exists():
                # Check if we have metadata
                metadata_path = tiff_path.with_suffix('.json')
                if metadata_path.exists():
                    logger.info(f"Using existing converted file: {tiff_path}")
                    self.converted_files[str(filepath)] = tiff_path
                    return self.load_tiff_with_metadata(tiff_path)
            
            # Perform conversion
            output_path, metadata = self.converter.nd2_to_tiff(filepath, tiff_path, save_metadata=True)
